...
test.describe('PDV @live — venda baixa estoque e protege preço ausente', () => {
  ...
  test('vende 3 unidades, mostra KG 0.500 e bloqueia produto sem preço', async ({ page, browser }) => {
    ...
    if (recoveryCode.length >= 8) await admin.locator('button[type="button"]').last().click()
    ...
    await expect(admin.getByTestId('balance-row').filter({ hasText: artifact.products.unit.name })).toContainText(/\b7\b/)
    await admin.goto(`/inventory/balances?q=${encodeURIComponent(artifact.products.kilogram.name)}`)
    await expect(admin.getByTestId('balance-row').filter({ hasText: artifact.products.kilogram.name })).toContainText('0.500kg')
