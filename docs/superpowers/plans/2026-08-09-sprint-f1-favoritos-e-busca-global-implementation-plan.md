# Sprint F1 — Favoritos e busca global Implementation Plan
...
**Goal:** entregar busca autorizada e favoritos persistentes.

**Architecture:** Search service consulta projeções por permissão; favoritos ficam tenant/user-scoped. UI usa combobox acessível.

**Tech Stack:** Django, DRF, PostgreSQL FTS, React Query
...
- Create or modify: `backend/tenancy/models.py` — contrato persistente ou declarativo da sprint.
- Create or modify: `backend/search/services.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_global_search.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/search/GlobalSearch.tsx` — superfície acessível ligada ao contrato.
...
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f1-favoritos-e-busca-global-design.md`
...
```python
def test_f1_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/search/?q=arroz', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert all(item['allowed'] for item in response.json()['results'])
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_global_search.py -q --no-cov`

Expected: FAIL because the F1 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_global_search.py docs/superpowers/specs/2026-08-09-sprint-f1-favoritos-e-busca-global-design.md
git commit -m "test(f1): define acceptance contract"
```

### Task 2: Implement the domain boundary

**Files:**
- Create or modify: `backend/tenancy/models.py`
- Create or modify: `backend/search/services.py`
- Test: `backend/tests/test_global_search.py`
...
class SprintF1Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f1_command(command: SprintF1Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/search/services.py`; persist the declared fields in `backend/tenancy/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/search/?q=arroz` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f1_acceptance_contract_rejects_invalid`, `test_f1_acceptance_contract_isolates_tenant`, and `test_f1_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_global_search.py -q --no-cov
...
```bash
git add backend/tenancy/models.py backend/search/services.py backend/tests/test_global_search.py backend/*/migrations
git commit -m "feat(f1): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/search/GlobalSearch.tsx`
...
```typescript
it('f1 renders Buscar no TJSys and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Buscar no TJSys/i)).toBeVisible()
  server.use(http.get(/f1/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```bash
git add frontend/src/search/GlobalSearch.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f1): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f1-favoritos-e-busca-global.spec.ts`
...
```typescript
test('f1 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Buscar no TJSys/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Buscar no TJSys/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/f1-favoritos-e-busca-global.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f1-favoritos-e-busca-global.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f1): close vertical acceptance"
```
