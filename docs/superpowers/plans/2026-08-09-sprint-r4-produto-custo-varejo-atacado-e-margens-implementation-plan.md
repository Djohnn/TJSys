# Sprint R4 — Produto custo varejo atacado e margens Implementation Plan
...
**Goal:** persistir preço efetivo e atacado com margem derivada.

**Architecture:** Pricing service resolve custo e um preço efetivo; tiers permanecem vinculados ao preço base. A UI nunca persiste margem duplicada.

**Tech Stack:** Django ORM, DRF, React Hook Form, Decimal.js
...
- Create or modify: `backend/catalog/services/pricing.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_product_redesign_pricing.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/catalog/ProductPricesStep.tsx` — superfície acessível ligada ao contrato.
...
**Files:**
- Test: `backend/tests/test_product_redesign_pricing.py`
- Reference: `docs/superpowers/specs/2026-08-09-sprint-r4-produto-custo-varejo-atacado-e-margens-design.md`
...
```python
def test_r4_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/catalog/products/{product_id}/prices/', headers=tenant_headers)
...
Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_product_redesign_pricing.py -q --no-cov`

Expected: FAIL because the R4 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_product_redesign_pricing.py docs/superpowers/specs/2026-08-09-sprint-r4-produto-custo-varejo-atacado-e-margens-design.md
git commit -m "test(r4): define acceptance contract"
```
...
- Create or modify: `backend/catalog/services/pricing.py`
- Test: `backend/tests/test_product_redesign_pricing.py`
...
Implement the exported boundary directly in `backend/catalog/services/pricing.py`; persist the declared fields in `backend/catalog/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/catalog/products/{product_id}/prices/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_r4_acceptance_contract_rejects_invalid`, `test_r4_acceptance_contract_isolates_tenant`, and `test_r4_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_product_redesign_pricing.py -q --no-cov
...
```bash
git add backend/catalog/models.py backend/catalog/services/pricing.py backend/tests/test_product_redesign_pricing.py backend/*/migrations
git commit -m "feat(r4): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/catalog/ProductPricesStep.tsx`
...
```typescript
it('r4 renders Venda varejo and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Venda varejo/i)).toBeVisible()
  server.use(http.get(/r4/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintR4Page() {
  return (
    <main aria-labelledby="r4-title">
      <h1 id="r4-title">Venda varejo</h1>
      <section aria-live="polite" data-testid="r4-content" />
    </main>
  )
}
```

Replace `data-testid="r4-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/catalog/ProductPricesStep.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(r4): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/r4-produto-custo-varejo-atacado-e-margens.spec.ts`
...
```typescript
test('r4 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Venda varejo/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Venda varejo/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/r4-produto-custo-varejo-atacado-e-margens.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/r4-produto-custo-varejo-atacado-e-margens.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(r4): close vertical acceptance"
```
