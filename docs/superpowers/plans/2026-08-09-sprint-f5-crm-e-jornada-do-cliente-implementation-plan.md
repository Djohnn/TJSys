# Sprint F5 — CRM e jornada do cliente Implementation Plan
...
**Goal:** registrar pipeline atividades e timeline autorizada.

**Architecture:** Novo app crm referencia Person sem duplicar PII. Timeline é projeção de eventos tenant-scoped.
...
- Create or modify: `backend/crm/models.py` — contrato persistente ou declarativo da sprint.
- Create or modify: `backend/crm/services.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_crm.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/crm/PipelinePage.tsx` — superfície acessível ligada ao contrato.
...
**Files:**
- Test: `backend/tests/test_crm.py`
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f5-crm-e-jornada-do-cliente-design.md`
...
```python
def test_f5_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/crm/opportunities/', headers=tenant_headers)
...
Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_crm.py -q --no-cov`

Expected: FAIL because the F5 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_crm.py docs/superpowers/specs/2026-08-09-sprint-f5-crm-e-jornada-do-cliente-design.md
git commit -m "test(f5): define acceptance contract"
```

### Task 2: Implement the domain boundary

**Files:**
- Create or modify: `backend/crm/models.py`
- Create or modify: `backend/crm/services.py`
- Test: `backend/tests/test_crm.py`
...
class SprintF5Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f5_command(command: SprintF5Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/crm/services.py`; persist the declared fields in `backend/crm/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/crm/opportunities/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f5_acceptance_contract_rejects_invalid`, `test_f5_acceptance_contract_isolates_tenant`, and `test_f5_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_crm.py -q --no-cov
...
```bash
git add backend/crm/models.py backend/crm/services.py backend/tests/test_crm.py backend/*/migrations
git commit -m "feat(f5): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/crm/PipelinePage.tsx`
...
```typescript
it('f5 renders Pipeline and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Pipeline/i)).toBeVisible()
  server.use(http.get(/f5/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintF5Page() {
  return (
    <main aria-labelledby="f5-title">
      <h1 id="f5-title">Pipeline</h1>
      <section aria-live="polite" data-testid="f5-content" />
    </main>
  )
}
```

Replace `data-testid="f5-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/crm/PipelinePage.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f5): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f5-crm-e-jornada-do-cliente.spec.ts`
...
```typescript
test('f5 vertical acceptance', async ({ page }) => {
...
Run: `cd frontend && npx playwright test e2e/f5-crm-e-jornada-do-cliente.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f5-crm-e-jornada-do-cliente.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f5): close vertical acceptance"
```
