# Sprint F8 — Compras avançadas Implementation Plan
...
**Goal:** comparar cotações e fechar devolução integrada.

**Architecture:** SupplierQuote converte para PurchaseOrder; devolução referencia receipt items e gera compensações atômicas.
...
- Create or modify: `backend/purchasing/models.py` — contrato persistente ou declarativo da sprint.
- Create or modify: `backend/purchasing/services.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_advanced_purchasing.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/purchasing/SupplierQuotesPage.tsx` — superfície acessível ligada ao contrato.
...
**Files:**
- Test: `backend/tests/test_advanced_purchasing.py`
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f8-compras-avancadas-design.md`
...
```python
def test_f8_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/purchasing/supplier-quotes/{quote_id}/convert/', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert PurchaseOrder.objects.filter(source_quote=quote).count() == 1
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_advanced_purchasing.py -q --no-cov`

Expected: FAIL because the F8 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_advanced_purchasing.py docs/superpowers/specs/2026-08-09-sprint-f8-compras-avancadas-design.md
git commit -m "test(f8): define acceptance contract"
```

### Task 2: Implement the domain boundary

**Files:**
- Create or modify: `backend/purchasing/models.py`
- Create or modify: `backend/purchasing/services.py`
- Test: `backend/tests/test_advanced_purchasing.py`
...
class SprintF8Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f8_command(command: SprintF8Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/purchasing/services.py`; persist the declared fields in `backend/purchasing/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/purchasing/supplier-quotes/{quote_id}/convert/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f8_acceptance_contract_rejects_invalid`, `test_f8_acceptance_contract_isolates_tenant`, and `test_f8_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_advanced_purchasing.py -q --no-cov
...
```bash
git add backend/purchasing/models.py backend/purchasing/services.py backend/tests/test_advanced_purchasing.py backend/*/migrations
git commit -m "feat(f8): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/purchasing/SupplierQuotesPage.tsx`
...
```typescript
it('f8 renders Comparar propostas and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Comparar propostas/i)).toBeVisible()
  server.use(http.get(/f8/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintF8Page() {
  return (
    <main aria-labelledby="f8-title">
      <h1 id="f8-title">Comparar propostas</h1>
      <section aria-live="polite" data-testid="f8-content" />
    </main>
  )
}
```

Replace `data-testid="f8-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/purchasing/SupplierQuotesPage.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f8): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f8-compras-avancadas.spec.ts`
...
```typescript
test('f8 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Comparar propostas/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Comparar propostas/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/f8-compras-avancadas.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f8-compras-avancadas.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f8): close vertical acceptance"
```
