# Sprint R6 — Produto contrato fiscal único Implementation Plan
...
**Goal:** consolidar dados fiscais em uma fonte canônica e migrável.

**Architecture:** ProductFiscalData recebe dados legados por migration auditável. Serializer de detalhe faz round-trip e valida códigos.

**Tech Stack:** Django migrations, DRF, React, pytest
...
- Create or modify: `backend/catalog/serializers.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_product_redesign_fiscal.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/catalog/ProductFiscalStep.tsx` — superfície acessível ligada ao contrato.
...
**Files:**
- Test: `backend/tests/test_product_redesign_fiscal.py`
- Reference: `docs/superpowers/specs/2026-08-09-sprint-r6-produto-contrato-fiscal-unico-design.md`
...
```python
def test_r6_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/catalog/products/{product_id}/fiscal-data/', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert response.json()['ncm'] == '22021000'
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_product_redesign_fiscal.py -q --no-cov`

Expected: FAIL because the R6 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_product_redesign_fiscal.py docs/superpowers/specs/2026-08-09-sprint-r6-produto-contrato-fiscal-unico-design.md
git commit -m "test(r6): define acceptance contract"
```
...
- Create or modify: `backend/catalog/serializers.py`
- Test: `backend/tests/test_product_redesign_fiscal.py`
...
class SprintR6Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_r6_command(command: SprintR6Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/catalog/serializers.py`; persist the declared fields in `backend/catalog/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/catalog/products/{product_id}/fiscal-data/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_r6_acceptance_contract_rejects_invalid`, `test_r6_acceptance_contract_isolates_tenant`, and `test_r6_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_product_redesign_fiscal.py -q --no-cov
...
```bash
git add backend/catalog/models.py backend/catalog/serializers.py backend/tests/test_product_redesign_fiscal.py backend/*/migrations
git commit -m "feat(r6): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/catalog/ProductFiscalStep.tsx`
...
```typescript
it('r6 renders NCM and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/NCM/i)).toBeVisible()
  server.use(http.get(/r6/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintR6Page() {
  return (
    <main aria-labelledby="r6-title">
      <h1 id="r6-title">NCM</h1>
      <section aria-live="polite" data-testid="r6-content" />
    </main>
  )
}
```

Replace `data-testid="r6-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/catalog/ProductFiscalStep.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(r6): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/r6-produto-contrato-fiscal-unico.spec.ts`
...
Run: `cd frontend && npx playwright test e2e/r6-produto-contrato-fiscal-unico.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/r6-produto-contrato-fiscal-unico.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(r6): close vertical acceptance"
```
