# Sprint F4 — Vendas avançadas Implementation Plan
...
**Goal:** entregar consignação comissão listas de preço e serviços.

**Architecture:** Resolvers determinísticos selecionam preço e comissão por vigência. Estorno projeta compensações, não apaga histórico.

**Tech Stack:** Django, DRF, React, Celery/outbox
...
- Create or modify: `backend/sales/pricing.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_advanced_sales.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/salesManagement/PriceListsPage.tsx` — superfície acessível ligada ao contrato.
...
**Files:**
- Test: `backend/tests/test_advanced_sales.py`
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f4-vendas-avancadas-design.md`
...
```python
def test_f4_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/sales/price-resolution/', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert response.json()['amount'] == '12.90'
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_advanced_sales.py -q --no-cov`

Expected: FAIL because the F4 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_advanced_sales.py docs/superpowers/specs/2026-08-09-sprint-f4-vendas-avancadas-design.md
git commit -m "test(f4): define acceptance contract"
```
...
- Create or modify: `backend/sales/pricing.py`
- Test: `backend/tests/test_advanced_sales.py`
...
class SprintF4Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f4_command(command: SprintF4Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/sales/pricing.py`; persist the declared fields in `backend/sales/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/sales/price-resolution/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f4_acceptance_contract_rejects_invalid`, `test_f4_acceptance_contract_isolates_tenant`, and `test_f4_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_advanced_sales.py -q --no-cov
...
```bash
git add backend/sales/models.py backend/sales/pricing.py backend/tests/test_advanced_sales.py backend/*/migrations
git commit -m "feat(f4): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/salesManagement/PriceListsPage.tsx`
...
```typescript
it('f4 renders Lista de preços and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Lista de preços/i)).toBeVisible()
  server.use(http.get(/f4/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintF4Page() {
  return (
    <main aria-labelledby="f4-title">
      <h1 id="f4-title">Lista de preços</h1>
      <section aria-live="polite" data-testid="f4-content" />
    </main>
  )
}
```

Replace `data-testid="f4-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/salesManagement/PriceListsPage.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f4): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f4-vendas-avancadas.spec.ts`
...
```typescript
test('f4 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Lista de preços/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Lista de preços/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/f4-vendas-avancadas.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f4-vendas-avancadas.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f4): close vertical acceptance"
```
