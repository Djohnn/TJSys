# Sprint F10 — Faturamento e documentos fiscais Implementation Plan
...
**Goal:** faturar pedidos e emitir uma única nota por comando.

**Architecture:** Billing service cria invoice e outbox fiscal atomicamente; webhook autenticado atualiza autorização.

**Tech Stack:** Django, DRF, Celery, outbox, React
...
- Create or modify: `backend/fiscal/models.py` — contrato persistente ou declarativo da sprint.
- Create or modify: `backend/fiscal/services.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_billing.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/fiscal/BillingPage.tsx` — superfície acessível ligada ao contrato.
...
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f10-faturamento-e-documentos-fiscais-design.md`
...
```python
def test_f10_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/fiscal/invoices/issue/', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert FiscalDocument.objects.filter(command_id=command_id).count() == 1
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_billing.py -q --no-cov`

Expected: FAIL because the F10 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_billing.py docs/superpowers/specs/2026-08-09-sprint-f10-faturamento-e-documentos-fiscais-design.md
git commit -m "test(f10): define acceptance contract"
```

### Task 2: Implement the domain boundary

**Files:**
- Create or modify: `backend/fiscal/models.py`
- Create or modify: `backend/fiscal/services.py`
- Test: `backend/tests/test_billing.py`
...
class SprintF10Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f10_command(command: SprintF10Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/fiscal/services.py`; persist the declared fields in `backend/fiscal/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/fiscal/invoices/issue/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f10_acceptance_contract_rejects_invalid`, `test_f10_acceptance_contract_isolates_tenant`, and `test_f10_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_billing.py -q --no-cov
...
```bash
git add backend/fiscal/models.py backend/fiscal/services.py backend/tests/test_billing.py backend/*/migrations
git commit -m "feat(f10): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/fiscal/BillingPage.tsx`
...
```typescript
it('f10 renders Emitir nota and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Emitir nota/i)).toBeVisible()
  server.use(http.get(/f10/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintF10Page() {
  return (
    <main aria-labelledby="f10-title">
      <h1 id="f10-title">Emitir nota</h1>
      <section aria-live="polite" data-testid="f10-content" />
    </main>
  )
}
```

Replace `data-testid="f10-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/fiscal/BillingPage.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f10): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f10-faturamento-e-documentos-fiscais.spec.ts`
...
```typescript
test('f10 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Emitir nota/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Emitir nota/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/f10-faturamento-e-documentos-fiscais.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f10-faturamento-e-documentos-fiscais.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f10): close vertical acceptance"
```
