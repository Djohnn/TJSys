# Sprint F9 — Financeiro bancário Implementation Plan
...
**Goal:** importar extratos e conciliar lançamentos sem duplicação.

**Architecture:** BankStatementEntry usa chave externa única e reconciliation service registra matches reversíveis.
...
- Create or modify: `backend/financial/models.py` — contrato persistente ou declarativo da sprint.
- Create or modify: `backend/financial/services.py` — regra canônica, sem lógica duplicada na UI.
- Create or modify: `backend/tests/test_bank_reconciliation.py` — regressão do cenário vertical.
- Create or modify: `frontend/src/financial/BankReconciliationPage.tsx` — superfície acessível ligada ao contrato.
...
- Reference: `docs/superpowers/specs/2026-08-09-sprint-f9-financeiro-bancario-design.md`
...
```python
def test_f9_acceptance_contract(api_client, tenant_headers):
    # Given an authenticated tenant and valid sprint fixture
    response = api_client.get('/api/v1/financial/bank-statements/import/', headers=tenant_headers)
    # When the canonical contract is requested
    assert response.status_code in {200, 201}
    # Then the vertical invariant is observable
    assert BankStatementEntry.objects.count() == imported_count
```

- [ ] **Step 2: Run the test and confirm RED**

Run: `cd backend && ..\\.venv\\Scripts\\python.exe -m pytest tests/test_bank_reconciliation.py -q --no-cov`

Expected: FAIL because the F9 contract, route or accessible control does not exist yet.

- [ ] **Step 3: Commit the RED test**

```bash
git add backend/tests/test_bank_reconciliation.py docs/superpowers/specs/2026-08-09-sprint-f9-financeiro-bancario-design.md
git commit -m "test(f9): define acceptance contract"
```
...
class SprintF9Command:
    tenant_id: UUID
    command_id: UUID
    payload: dict[str, object]

def execute_f9_command(command: SprintF9Command) -> dict[str, object]:
...
Implement the exported boundary directly in `backend/financial/services.py`; persist the declared fields in `backend/financial/models.py`; create the numbered migration plus PostgreSQL RLS for every tenant table; and register `/api/v1/financial/bank-statements/import/` in the app's existing serializer/view/router files.

- [ ] **Step 2: Prove negative, tenant and replay behavior**

Add three named tests: `test_f9_acceptance_contract_rejects_invalid`, `test_f9_acceptance_contract_isolates_tenant`, and `test_f9_acceptance_contract_replays_command_id`. Each must assert response and side-effect counts.
...
```powershell
cd backend
..\.venv\Scripts\python.exe -m pytest tests/test_bank_reconciliation.py -q --no-cov
...
```bash
git add backend/financial/models.py backend/financial/services.py backend/tests/test_bank_reconciliation.py backend/*/migrations
git commit -m "feat(f9): implement domain contract"
```

### Task 3: Implement the accessible UI slice

**Files:**
- Create or modify: `frontend/src/financial/BankReconciliationPage.tsx`
...
```typescript
it('f9 renders Conciliar and reports API errors', async () => {
  render(<TestApp initialEntry="/" />)
  expect(await screen.findByText(/Conciliar/i)).toBeVisible()
  server.use(http.get(/f9/, () => HttpResponse.json({ title: 'Falha controlada' }, { status: 422 })))
...
```tsx
export function SprintF9Page() {
  return (
    <main aria-labelledby="f9-title">
      <h1 id="f9-title">Conciliar</h1>
      <section aria-live="polite" data-testid="f9-content" />
    </main>
  )
}
```

Replace `data-testid="f9-content"` with the page's typed query and form components named in the file map, including explicit loading, empty, Problem Details error, 409 conflict and success states. Use tokens from `frontend/src/styles/tokens.css`; no literal HEX. Keep navigation `planned` until component and backend gates are green.
...
```bash
git add frontend/src/financial/BankReconciliationPage.tsx frontend/src/app/App.tsx frontend/src/layout/navigationModel.ts frontend/src
git commit -m "feat(f9): expose accessible workflow"
```

### Task 4: Close the vertical acceptance gate

**Files:**
- Create or modify: `frontend/e2e/f9-financeiro-bancario.spec.ts`
...
```typescript
test('f9 vertical acceptance', async ({ page }) => {
  // Given seeded tenant data and an authenticated storage state
  await page.goto('/')
  // When the user opens the real route
  await page.getByRole('link', { name: /Conciliar/i }).click()
  // Then the sprint surface is usable without retries
  await expect(page.getByRole('heading', { name: /Conciliar/i })).toBeVisible()
})
```
...
Run: `cd frontend && npx playwright test e2e/f9-financeiro-bancario.spec.ts --project=chromium --retries=0`
...
```bash
git add frontend/e2e/f9-financeiro-bancario.spec.ts .github/workflows/e2e.yml docs/DOCUMENT_INDEX.md graphify-out
git commit -m "test(f9): close vertical acceptance"
```
