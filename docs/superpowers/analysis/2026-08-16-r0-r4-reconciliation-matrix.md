...
| R1 — tokens e componentes fundamentais | `frontend/src/design-system/tokens.ts`; `frontend/src/styles/tokens.css`; `frontend/src/components/ui/Button.tsx`; `frontend/src/components/ui/Input.tsx`; `frontend/src/components/ui/designSystem.test.tsx`; `frontend/e2e/r1-tokens-e-componentes-fundamentais.spec.ts`; `frontend/package.json`; `frontend/package-lock.json`; `frontend/src/test/setup.ts`; `docs/DOCUMENT_INDEX.md` | `bb375bd..a8d447d` (`32f29d7`, `e5d4fa6`, `317f26d`, `a8d447d`) | incorporate | Merge sem conflitos (`git merge --no-ff --no-commit a8d447d`). A fonte semântica é `tokens.ts`/`tokens.css`; a API nomeada de `Button` foi preservada e `Input` mantém `label` obrigatório. A fixture visual R1 permanece deferida à R2/Task4; ProductStockFields/inventoryApi permanecem intocados e deferidos à R3/Task5. |
...
* **Proveniência:** a diferença integrada é exclusivamente `bb375bd..a8d447d`: `32f29d7` (contrato), `e5d4fa6` (export nomeado/dependência axe), `317f26d` (tokens, Button/Input e setup) e `a8d447d` (spec visual e índice documental).
* **Arquivos integrados:** `docs/DOCUMENT_INDEX.md`; `frontend/package.json`; `frontend/package-lock.json`; `frontend/src/components/ui/Button.tsx`; `frontend/src/components/ui/Input.tsx`; `frontend/src/components/ui/designSystem.test.tsx`; `frontend/src/design-system/tokens.ts`; `frontend/src/styles/tokens.css`; `frontend/src/test/setup.ts`; `frontend/e2e/r1-tokens-e-componentes-fundamentais.spec.ts`.
...
* **Decisões:** consumidores existentes mantêm as APIs atuais; `Button` expõe export nomeado e default; `Input` associa `label` ao controle; cores semânticas de R1 ficam centralizadas nos tokens. Componentes/contratos de R2–R4, a fixture visual R1 e o typecheck de `ProductStockFields`/`inventoryApi` não entram nesta consolidação.
* **Limite de escopo:** `Select`, `Textarea`, `Switch`, `Tabs` e `Alert` não fazem parte da diferença `bb375bd..a8d447d` nem existem no baseline deste worktree; não foram criados nesta task para evitar introduzir R2+ ou uma alteração fora da proveniência R1. Essa lacuna permanece registrada para a frente de componentes correspondente.
...
* **Detector mecânico (output bruto):** `node C:\ERP\.agents\skills\impeccable\scripts\detect.mjs --json frontend/src/design-system frontend/src/components/ui frontend/src/styles/tokens.css` → `[]` (exit `0`).
* **Audit manual (escopo alterado):** sem cores HEX literais novas fora dos arquivos de tokens permitidos (checker passou); `Button` é `<button>` e `Input` associa `label`/`htmlFor`; foco visível está declarado no Button. O componente `Modal` existente não possui `role="dialog"`, `aria-modal` ou foco aprisionado no baseline R0; Select/Textarea/Switch/Tabs/Alert não existem no delta R1. Isso é uma lacuna P1 de acessibilidade para a frente de componentes, não corrigida nesta consolidação restrita.
* **Polish manual:** preservadas APIs/consumidores e nenhum ajuste visual fora da proveniência R1; tokens CSS foram mantidos como fonte declarativa do delta e a fixture visual R1 continua deferida à R2/Task4. Não houve snapshot nem redesign.
