...
* **Proveniência:** `bb375bd..a8d447d` fornece o slice R1 original; `0f085bb` registra o RED completo; a implementação GREEN fica no commit seguinte permitido `fix(r1): complete tokens and primitives`.
...
* **Decisões:** consumidores existentes mantêm APIs; todos os primitives têm export nomeado/default; `Input`, `Select` e `Textarea` geram IDs estáveis e associam erros; `Modal` usa dialog nomeado, `aria-modal`, Escape, foco inicial, trap básico e retorno; logos são servidos em paths estáveis e preservam os hashes do manifest. Componentes/contratos de R2–R4, fixture R1 e typecheck de `ProductStockFields`/`inventoryApi` não entram nesta consolidação.
