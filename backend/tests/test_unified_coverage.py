...
def uc(client):
    ...
    def _create():
        ...
        company = Company.all_objects.create(
            tenant=tenant, name='Empresa UC', cnpj='11223344000155'
        )
...
class TestDeactivateProductStockControl:
    ...
    def test_payload_mismatch(self, uc):
        ...
        ProductStockControlCommand.all_objects.create(
            ...
            action='deactivate',
            payload_hash='wrong',
            response_json={'policy_updated': 0},
...
class TestApplyProviderQueryResult:
    ...
    def test_concluded(self, uc):
        ...
        result = QueryResult(
            status='CONCLUIDO', protocol='P123', xml_url='xml', pdf_url='pdf', error_reason=None
        )
    ...
    def test_rejeitado(self, uc):
        ...
        result = QueryResult(
            status='REJEITADO', protocol=None, xml_url=None, pdf_url=None, error_reason='Erro'
        )
        doc = apply_provider_query_result(doc, result)
    ...
    def test_cancelado(self, uc):
        ...
        result = QueryResult(
            status='CANCELADO', protocol=None, xml_url=None, pdf_url=None, error_reason='X'
        )
        doc = apply_provider_query_result(doc, result)
    ...
    def test_processing(self, uc):
        ...
        result = QueryResult(
            status='PROCESSANDO', protocol=None, xml_url=None, pdf_url=None, error_reason=None
        )
        doc = apply_provider_query_result(doc, result)
...
class TestHasCapability:
    ...
    def test_capability_not_allowed(self, uc):
        ...
        request.tenant = uc['tenant']
        request.user = uc['user']
        TenantMembership.objects.filter(user=uc['user'], tenant=uc['tenant']).update(
            role='operator'
        )
        assert perm.has_permission(request, view) is False
