"""Final coverage push — product_stock services, tenancy views, platform_admin views, session views."""
...
class TestComputeStockStatus:
    ...
    def test_negative(self):
        ...
        assert compute_stock_status(available=Decimal('-5'), reorder_point=Decimal('10')) == 'negative'
    ...
    def test_normal(self):
        from inventory.services.product_stock import compute_stock_status
        assert compute_stock_status(available=Decimal('50'), reorder_point=Decimal('10')) == 'normal'
...
class TestBuildProductStockSummary:
    ...
    def test_with_balance(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='SumCo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='SumBr'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade', precision=2))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='SUM-1', name='SumProd', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='S1', name='Dep S1'))
        policy = _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('5'), maximum_quantity=Decimal('50'),
            reorder_point=Decimal('10'), allow_negative=False,
        ))
        balance = _run_in_tenant(tenant, lambda: StockBalance.objects.create(
            tenant=tenant, product=product, location=location,
            quantity=Decimal('20'), reserved=Decimal('3'),
        ))
    ...
    def test_without_balance(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='SumNoCo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='SumNoBr'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='SUMNO-1', name='SumNoProd', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='SN1', name='Dep SN1'))
        policy = _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('5'), allow_negative=False,
        ))
...
class TestDeactivateProductStockControl:
    ...
    def test_missing_command_id_raises(self):
        ...
        with pytest.raises(ProductStockControlError, match='command_id é obrigatório'):
            deactivate_product_stock_control(tenant=tenant, product=product, actor=None, command_id='')
    ...
    def test_deactivate_with_zero_balances(self):
        ...
        user = User.objects.create_user(email=f'dpc2-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DPC2Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DPC2Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='DPC2-1', name='DPC2P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='D2', name='Dep D2'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False,
        ))
        _run_in_tenant(tenant, lambda: StockBalance.objects.create(
            tenant=tenant, product=product, location=location,
            quantity=Decimal('0'), reserved=Decimal('0'),
        ))
        result = _run_in_tenant(tenant, lambda: deactivate_product_stock_control(
            tenant=tenant, product=product, actor=user,
            command_id=str(uuid.uuid4()), correlation_id=str(uuid.uuid4()),
        ))
    ...
    def test_idempotent_retry_same_payload(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DPC3Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DPC3Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='DPC3-1', name='DPC3P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='D3', name='Dep D3'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False,
        ...
        corr = str(uuid.uuid4())
        r1 = _run_in_tenant(tenant, lambda: deactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=cmd_id, correlation_id=corr,
        ))
        r2 = _run_in_tenant(tenant, lambda: deactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=cmd_id, correlation_id=corr,
        ))
    ...
    def test_conflict_on_different_payload(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DPC4Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DPC4Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='DPC4-1', name='DPC4P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='D4', name='Dep D4'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False,
        ...
        cmd_id = str(uuid.uuid4())
        product2 = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='DPC4-2', name='DPC4P2', base_unit=unit))
        _run_in_tenant(tenant, lambda: deactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=cmd_id, correlation_id=str(uuid.uuid4()),
        ))
        with pytest.raises(ProductStockControlError, match='payload diferente'):
            _run_in_tenant(tenant, lambda: deactivate_product_stock_control(
                tenant=tenant, product=product2, actor=None,
                command_id=cmd_id, correlation_id=str(uuid.uuid4()),
            ))
    ...
    def test_no_correlation_generates_one(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DPC5Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DPC5Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='DPC5-1', name='DPC5P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='D5', name='Dep D5'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False,
...
class TestReactivateProductStockControl:
    ...
    def test_missing_command_id_raises(self):
        ...
        product = _run_in_tenant(tenant, lambda: type('P', (), {'id': uuid.uuid4()})())
        with pytest.raises(ProductStockControlError, match='command_id é obrigatório'):
            reactivate_product_stock_control(tenant=tenant, product=product, actor=None, command_id='')
    ...
    def test_no_policies_raises(self):
        ...
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC2-1', name='RPC2P', base_unit=unit))
        with pytest.raises(ProductStockControlError, match='Nenhuma política'):
            _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
                tenant=tenant, product=product, actor=None,
                command_id=str(uuid.uuid4()), initial_stocks=[],
            ))
    ...
    def test_reactivate_no_initial_stocks(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC3Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC3Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC3-1', name='RPC3P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R3', name='Dep R3'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
        ))
        result = _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=str(uuid.uuid4()), correlation_id=str(uuid.uuid4()),
        ))
    ...
    def test_idempotent_reactivate(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC4Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC4Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC4-1', name='RPC4P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R4', name='Dep R4'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
        ...
        cmd_id = str(uuid.uuid4())
        r1 = _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=cmd_id, correlation_id=str(uuid.uuid4()),
        ))
        r2 = _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
            tenant=tenant, product=product, actor=None,
            command_id=cmd_id, correlation_id=str(uuid.uuid4()),
        ))
        assert r1.correlation_id == r2.correlation_id
    ...
    def test_reactivate_conflict_different_payload(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC5Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC5Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC5-1', name='RPC5P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R5', name='Dep R5'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
        ...
        loc2 = _run_in_tenant(tenant, lambda: StockLocation.objects.create(
            tenant=tenant, branch=branch, code='R5b', name='Dep R5b',
        ))
        with pytest.raises(ProductStockControlError, match='payload diferente'):
            _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
                tenant=tenant, product=product, actor=None,
    ...
    def test_invalid_initial_stock_not_dict(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC6Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC6Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC6-1', name='RPC6P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R6', name='Dep R6'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
        ...
        with pytest.raises(ProductStockControlError, match='Estoque inicial inválido'):
            _run_in_tenant(tenant, lambda: reactivate_product_stock_control(
                tenant=tenant, product=product, actor=None,
                command_id=str(uuid.uuid4()), initial_stocks=['not-a-dict'],
            ))
    ...
    def test_invalid_location_id(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC7Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC7Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC7-1', name='RPC7P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R7', name='Dep R7'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
    ...
    def test_invalid_quantity(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC8Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC8Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC8-1', name='RPC8P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R8', name='Dep R8'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
    ...
    def test_negative_quantity(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC9Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC9Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC9-1', name='RPC9P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R9', name='Dep R9'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
    ...
    def test_location_not_in_policies(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC10Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC10Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC10-1', name='RPC10P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R10', name='Dep R10'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
        ))
        other_loc = _run_in_tenant(tenant, lambda: StockLocation.objects.create(
            tenant=tenant, branch=branch, code='R10b', name='Dep R10b',
        ))
    ...
    def test_no_correlation_generates_one(self):
        ...
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='RPC11Co'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='RPC11Br'))
        unit = _run_in_tenant(tenant, lambda: Unit.all_objects.create(tenant=tenant, symbol='UN', name='Unidade'))
        product = _run_in_tenant(tenant, lambda: Product.all_objects.create(tenant=tenant, sku='RPC11-1', name='RPC11P', base_unit=unit))
        location = _run_in_tenant(tenant, lambda: StockLocation.objects.create(tenant=tenant, branch=branch, code='R11', name='Dep R11'))
        _run_in_tenant(tenant, lambda: ProductStockPolicy.objects.create(
            tenant=tenant, product=product, branch=branch, location=location,
            minimum_quantity=Decimal('0'), maximum_quantity=None,
            reorder_point=Decimal('0'), allow_negative=False, is_active=False,
...
class TestCompanyListCreateView:
    ...
    def test_list_companies(self, client):
        ...
        admin = User.objects.create_user(email=f'co-l-{uuid.uuid4().hex[:6]}@test.local', password='pass')
    ...
    def test_create_company(self, client):
        ...
        admin = User.objects.create_user(email=f'co-c-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        resp = client.post(
            '/api/v1/companies/',
            {'name': 'NewCo'},
...
class TestCompanyDetailView:
    ...
    def test_retrieve_company(self, client):
        ...
        admin = User.objects.create_user(email=f'co-d-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
...
class TestDeviceRegisterView:
    ...
    def test_register_device(self, client):
        ...
        admin = User.objects.create_user(email=f'dvr-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVRCo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DVRBr'))
        resp = client.post(
            '/api/v1/devices/',
...
class TestDeviceValidateView:
    ...
    def test_validate_valid_key(self, client):
        ...
        admin = User.objects.create_user(email=f'dvv-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVVCo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DVVBr'))
        api_key = 'device-api-key-123'
        _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=branch, name='DV', device_id='dv-1',
            key_hash=hashlib.sha256(api_key.encode()).hexdigest(),
            status='active', registered_by=admin,
        ))
        resp = client.post(
    ...
    def test_validate_inactive_device(self, client):
        ...
        admin = User.objects.create_user(email=f'dvi-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVICo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DVIBr'))
        api_key = 'inactive-key'
        _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=branch, name='DVI', device_id='dvi-1',
            key_hash=hashlib.sha256(api_key.encode()).hexdigest(),
            status='inactive', registered_by=admin,
        ))
        resp = client.post(
            '/api/v1/devices/validate/',
            {'api_key': api_key},
...
class TestDeviceRefreshView:
    ...
    def test_refresh_missing_token(self, client):
        ...
        admin = User.objects.create_user(email=f'dvrf-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        resp = client.post(
    ...
    def test_refresh_valid_token(self, client):
        ...
        admin = User.objects.create_user(email=f'dvrv-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        company = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVRvCo'))
        branch = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=company, name='DVRvBr'))
        device = _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=branch, name='DVRv', device_id='dvrv-1',
            key_hash='x', status='active', registered_by=admin,
        ))
...
class TestBranchListCreateView:
    ...
    def test_list_branches(self, client):
        ...
        admin = User.objects.create_user(email=f'br-l-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
    ...
    def test_create_branch(self, client):
        ...
        admin = User.objects.create_user(email=f'br-c-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
...
class TestBranchDetailView:
    ...
    def test_retrieve_branch(self, client):
        ...
        admin = User.objects.create_user(email=f'br-d-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        co = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='BrDCo'))
        br = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=co, name='BrD'))
    ...
    def test_patch_branch(self, client):
        ...
        admin = User.objects.create_user(email=f'br-p-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        co = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='BrPCo'))
        br = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=co, name='BrP'))
        resp = client.patch(
...
class TestDeviceListView:
    ...
    def test_list_devices(self, client):
        ...
        admin = User.objects.create_user(email=f'dvl-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        co = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVLCo'))
        br = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=co, name='DVLBr'))
        _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=br, name='DVL', device_id='dvl-1',
            key_hash='x', status='active', registered_by=admin,
        ))
...
class TestDeviceRevokeView:
    ...
    def test_revoke_device(self, client):
        ...
        admin = User.objects.create_user(email=f'dvrk-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        co = _run_in_tenant(tenant, lambda: Company.objects.create(tenant=tenant, name='DVRkCo'))
        br = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=co, name='DVRkBr'))
        device = _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=br, name='DVRk', device_id='dvrk-1',
            key_hash='x', status='active', registered_by=admin,
        ))
    ...
    def test_revoke_not_found(self, client):
        ...
        admin = User.objects.create_user(email=f'dvrk404-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
        resp = client.post(
    ...
    def test_revoke_insufficient_role(self, client):
        ...
        operator = User.objects.create_user(email=f'dvrkdeny-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        ...
        br = _run_in_tenant(tenant, lambda: Branch.objects.create(tenant=tenant, company=co, name='DVRkDBr'))
        device = _run_in_tenant(tenant, lambda: Device.all_objects.create(
            tenant=tenant, branch=br, name='DVRkD', device_id='dvrkd-1',
            key_hash='x', status='active', registered_by=operator,
        ))
        resp = client.post(
...
class TestPlanViewSet:
    ...
    def test_list_plans(self, client):
        user = User.objects.create_user(email=f'pa-pl-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
    ...
    def test_create_plan(self, client):
        user = User.objects.create_user(email=f'pa-pc-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        resp = client.post(
    ...
    def test_update_plan(self, client):
        user = User.objects.create_user(email=f'pa-pu-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        from platform_admin.models import Plan
    ...
    def test_delete_plan(self, client):
        user = User.objects.create_user(email=f'pa-pd-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        from platform_admin.models import Plan
    ...
    def test_non_staff_denied(self, client):
        user = User.objects.create_user(email=f'pa-pns-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        client.force_login(user)
        resp = client.get('/api/v1/platform/plans/')
        assert resp.status_code == 403
...
class TestSubscriptionViewSet:
    ...
    def test_list_subscriptions(self, client):
        user = User.objects.create_user(email=f'pa-sl-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
    ...
    def test_suspend_action(self, client):
        ...
        user = User.objects.create_user(email=f'pa-sus-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
    ...
    def test_reactivate_action(self, client):
        ...
        user = User.objects.create_user(email=f'pa-react-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
...
class TestTenantEntitlementViewSet:
    ...
    def test_list_entitlements(self, client):
        user = User.objects.create_user(email=f'pa-te-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        resp = client.get('/api/v1/platform/entitlements/')
        assert resp.status_code == 200

    def test_create_entitlement(self, client):
        user = User.objects.create_user(email=f'pa-tec-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
    ...
    def test_filter_by_tenant(self, client):
        ...
        user = User.objects.create_user(email=f'pa-tef-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
...
class TestFeatureFlagViewSet:
    ...
    def test_list_flags(self, client):
        user = User.objects.create_user(email=f'pa-ff-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        resp = client.get('/api/v1/platform/feature-flags/')
        assert resp.status_code == 200

    def test_create_flag(self, client):
        user = User.objects.create_user(email=f'pa-ffc-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
        resp = client.post(
    ...
    def test_delete_flag(self, client):
        ...
        user = User.objects.create_user(email=f'pa-ffd-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
...
class TestSupportAccessViewSet:
    ...
    def _staff_client(self, client):
        user = User.objects.create_user(email=f'pa-sa-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
    ...
    def test_create_support_request(self, client):
        user = User.objects.create_user(email=f'pa-sac-{uuid.uuid4().hex[:6]}@test.local', password='pass')
    ...
    def test_approve_support_request(self, client):
        ...
        req = SupportAccessRequest.objects.create(
            requester=user, target_tenant=target, reason='Test',
        )
        resp = client.post(
    ...
    def test_approve_non_staff_forbidden(self, client):
        user = User.objects.create_user(email=f'pa-sans-{uuid.uuid4().hex[:6]}@test.local', password='pass')
    ...
    def test_revoke_non_staff_forbidden(self, client):
        user = User.objects.create_user(email=f'pa-sarvns-{uuid.uuid4().hex[:6]}@test.local', password='pass')
...
class TestPlatformAdminAuditViewSet:
    ...
    def test_list_audits(self, client):
        user = User.objects.create_user(email=f'pa-audit-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
    ...
    def test_filter_by_action(self, client):
        ...
        user = User.objects.create_user(email=f'pa-audita-{uuid.uuid4().hex[:6]}@test.local', password='pass', is_staff=True)
        client.force_login(user)
...
class TestTenantCapabilitiesView:
    ...
    def test_list_capabilities_no_subscription(self, client):
        ...
        admin = User.objects.create_user(email=f'tcap-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
    ...
    def test_list_capabilities_with_subscription(self, client):
        ...
        admin = User.objects.create_user(email=f'tcap2-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        _auth_client(client, admin, tenant)
...
class TestMeView:
    ...
    def test_me_returns_user_info(self, client):
        user = User.objects.create_user(email=f'me-{uuid.uuid4().hex[:6]}@test.local', password='pass')
    ...
    def test_me_excludes_inactive_memberships(self, client):
        user = User.objects.create_user(email=f'mea-{uuid.uuid4().hex[:6]}@test.local', password='pass')
    ...
    def test_me_multiple_memberships(self, client):
        user = User.objects.create_user(email=f'mem-{uuid.uuid4().hex[:6]}@test.local', password='pass')
...
class TestLogoutViewEdge:
    ...
    def test_logout_creates_audit(self, client):
        user = User.objects.create_user(email=f'logout-audit-{uuid.uuid4().hex[:6]}@test.local', password='pass')
        client.force_login(user)
        resp = client.post('/api/v1/auth/logout/')
        assert resp.status_code == 204
        from audit.models import AuditRecord
        assert AuditRecord.objects.filter(
            actor=user, action='auth.logout',
        ).exists()
