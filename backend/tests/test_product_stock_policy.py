...
def test_policy_unique_per_tenant_product_branch_location():
    """Given an existing policy, a duplicate raises IntegrityError."""
...
def test_backfill_migration_creates_zero_limit_policy_per_location():
    ...
    apps = executor.loader.project_state(historical_targets).apps
    ...
    executor.migrate(migrated_targets)
    apps = executor.loader.project_state(migrated_targets).apps
