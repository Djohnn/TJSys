...
def _prd(tenant, sku=None, unit=None):
    ...
    return _run_in_tenant(
        ...
        lambda: Product.all_objects.get_or_create(
            tenant=tenant,
            sku=sku,
            defaults={'name': sku, 'base_unit': unit},
        )[0],
    )
...
def _loc(tenant, branch, code='LOC'):
    ...
    return _run_in_tenant(
        tenant,
        lambda: StockLocation.all_objects.get_or_create(
            tenant=tenant,
            branch=branch,
            code=code,
            defaults={'name': code, 'is_primary': True},
        )[0],
    )
...
class TestStockLocationClean:
    def test_delete_with_movements_raises(self):
        ...
        op = StockOperation.all_objects.create(
            tenant=t, operation_type='adjustment', status='draft'
        )
...
class TestStockLotClean:
    def test_expiry_before_manufacture(self):
        ...
        t = _t()
        product = _prd(t, 'LOT-P')
        lot = StockLot(
            tenant=t,
            product=product,
            lot_number='LOT1',
            manufacture_date=date(2025, 12, 31),
    ...
    def test_is_expired(self):
        ...
        t = _t()
        product = _prd(t, 'LOT-EXP')
        lot = StockLot.all_objects.create(
            tenant=t, product=product, lot_number='E', expiry_date=date(2020, 1, 1)
        )
        assert lot.is_expired is True
        lot2 = StockLot.all_objects.create(
            tenant=t, product=product, lot_number='F', expiry_date=date(2099, 12, 31)
        )
    ...
    def test_duplicate_idempotency_key(self):
        ...
        def _c():
            StockOperation.all_objects.create(
                tenant=t, operation_type='receipt', idempotency_key='DK'
            )
...
class TestStockMovementClean:
    ...
    def test_immutable_on_save(self):
        ...
        loc = _loc(t, _b(t))
        op = StockOperation.all_objects.create(
            tenant=t, operation_type='adjustment', status='confirmed'
        )
...
class TestStockBalanceClean:
    ...
    def test_available(self):
        ...
        loc = _loc(t, _b(t))
        bal = StockBalance(
            tenant=t, product=product, location=loc, quantity=Decimal('10'), reserved=Decimal('3')
        )
    ...
    def test_str_with_lot(self):
        ...
        assert 'BL1' in str(
            StockBalance(tenant=t, product=product, location=loc, lot=lot, quantity=5)
        )
...
class TestStockOperationReversalClean:
    def test_cross_tenant_original(self):
        ...
        t2 = _t('trv2')
        op1 = _run_in_tenant(
            t, lambda: StockOperation.all_objects.create(tenant=t, operation_type='receipt')
        )
        op2 = _run_in_tenant(
            t, lambda: StockOperation.all_objects.create(tenant=t, operation_type='reversal')
        )
        with pytest.raises(ValidationError):
            StockOperationReversal(
                tenant=t2, original_operation=op1, reversal_operation=op2, reason='x'
            ).clean()
    ...
    def test_cross_tenant_reversal(self):
        ...
        op2 = _run_in_tenant(
            t2, lambda: StockOperation.all_objects.create(tenant=t2, operation_type='reversal')
        )
        with pytest.raises(ValidationError):
            StockOperationReversal(
                tenant=t, original_operation=op1, reversal_operation=op2, reason='x'
            ).clean()
...
class TestProductStockPolicyClean:
    def test_service_product(self):
        ...
        product = Product.all_objects.create(
            tenant=t, sku='SVC', name='S', base_unit=unit, product_kind='servico'
        )
        branch = _b(t)
        loc = _loc(t, branch)
        with pytest.raises(ValidationError):
    ...
    def test_negative_minimum(self):
        ...
        t = _t()
        product = _prd(t, 'PSP-NM')
        branch = _b(t)
        loc = _loc(t, branch)
        with pytest.raises(ValidationError):
            ProductStockPolicy(
                tenant=t, product=product, branch=branch, location=loc, minimum_quantity=-1
            ).clean()
    ...
    def test_negative_reorder(self):
        ...
        t = _t()
        product = _prd(t, 'PSP-NR')
        branch = _b(t)
        loc = _loc(t, branch)
        with pytest.raises(ValidationError):
            ProductStockPolicy(
                tenant=t, product=product, branch=branch, location=loc, reorder_point=-1
            ).clean()
    ...
    def test_cross_tenant_branch(self):
        ...
        t = _t()
        t2 = _t('tpsp3')
        product = _prd(t, 'PSP-XB')
        with pytest.raises(ValidationError):
            ProductStockPolicy(
                tenant=t, product=product, branch=_b(t2), location=_loc(t, _b(t))
            ).clean()
    ...
    def test_cross_tenant_location(self):
        ...
        t = _t()
        t2 = _t('tpsp4')
        product = _prd(t, 'PSP-XL')
        with pytest.raises(ValidationError):
            ProductStockPolicy(
                tenant=t, product=product, branch=_b(t), location=_loc(t2, _b(t2))
            ).clean()
...
class TestCashMovementClean:
    def test_negative_amount(self):
        ...
        user = _u(t)
        branch = _b(t)
        cs = CashSession.all_objects.create(
            tenant=t, branch=branch, operator=user, opening_amount=0
        )
    ...
    def test_cross_tenant_session(self):
        ...
        t = _t()
        t2 = _t('tcm2')
        user2 = _u(t2, 'cm2@t.co')
        cs = CashSession.all_objects.create(
            tenant=t2, branch=_b(t2), operator=user2, opening_amount=0
        )
        with pytest.raises(ValidationError):
...
class TestSaleItemClean:
    ...
    def test_negative_discount(self):
        ...
        with pytest.raises(ValidationError):
            SaleItem(
                tenant=_t(), quantity=1, factor=1, unit_price=1, line_total=0, discount_amount=-1
            ).clean()
...
class TestSaleReturnClean:
    def test_cross_tenant_sale(self):
        ...
        t = _t()
        t2 = _t('tsr2')
        user2 = _u(t2, 'sr2@t.co')
        cs = CashSession.all_objects.create(
            tenant=t2, branch=_b(t2), operator=user2, opening_amount=0
        )
        sale = Sale.all_objects.create(
            tenant=t2, branch=_b(t2), cash_session=cs, operator=user2, status='confirmed'
        )
        with pytest.raises(ValidationError):
...
class TestPurchaseOrderClean:
    def test_cross_tenant_supplier(self):
        ...
        with pytest.raises(ValidationError):
            PurchaseOrder(
                tenant=t, supplier=Supplier.all_objects.create(tenant=t2, name='S2'), branch=_b(t)
            ).clean()
    ...
    def test_cross_tenant_branch(self):
        ...
        with pytest.raises(ValidationError):
            PurchaseOrder(
                tenant=t, supplier=Supplier.all_objects.create(tenant=t, name='S3'), branch=_b(t2)
            ).clean()
    ...
    def test_str(self):
        ...
        t = _t()
        sup = Supplier.all_objects.create(tenant=t, name='Fornecedor')
...
class TestPurchaseOrderItemClean:
    ...
    def test_line_total(self):
        ...
        poi = PurchaseOrderItem(
            tenant=_t(), quantity=Decimal('3'), unit_cost=Decimal('10'), factor=Decimal('2')
        )
...
class TestPurchaseReceiptClean:
    ...
    def test_str(self):
        ...
        t = _t()
        sup = Supplier.all_objects.create(tenant=t, name='Forn')
        po = PurchaseOrder.all_objects.create(tenant=t, supplier=sup, branch=_b(t))
...
class TestPurchaseReceiptItemClean:
    ...
    def test_line_total(self):
        ...
        pri = PurchaseReceiptItem(
            tenant=_t(), quantity_received=Decimal('5'), unit_cost=Decimal('3')
        )
...
class TestPurchaseOrderCancellationClean:
    ...
    def test_str(self):
        ...
        sup = Supplier.all_objects.create(tenant=t, name='Forn')
        po = PurchaseOrder.all_objects.create(tenant=t, supplier=sup, branch=_b(t))
        poc = PurchaseOrderCancellation.all_objects.create(
            tenant=t, purchase_order=po, reason='x', idempotency_key=f'pc-{uuid.uuid4()}'
        )
...
class TestPurchaseReceiptCancellationClean:
    ...
    def test_str(self):
        ...
        prc = PurchaseReceiptCancellation.all_objects.create(
            tenant=t, receipt=pr, reason='x', idempotency_key=f'prc-{uuid.uuid4()}'
        )
...
class TestSupplierReturnClean:
    ...
    def test_str(self):
        ...
        po = PurchaseOrder.all_objects.create(tenant=t, supplier=sup, branch=_b(t))
        pr = PurchaseReceipt.all_objects.create(tenant=t, purchase_order=po)
        sr = SupplierReturn.all_objects.create(
            tenant=t, receipt=pr, reason='x', idempotency_key=f'sr-{uuid.uuid4()}'
        )
...
class TestRecurringTemplateClean:
    def test_cross_tenant_supplier(self):
        ...
        with pytest.raises(ValidationError):
            RecurringPurchaseOrderTemplate(
                tenant=t, supplier=Supplier.all_objects.create(tenant=t2, name='S'), branch=_b(t)
            ).clean()
    ...
    def test_cross_tenant_branch(self):
        ...
        with pytest.raises(ValidationError):
            RecurringPurchaseOrderTemplate(
                tenant=t, supplier=Supplier.all_objects.create(tenant=t, name='S'), branch=_b(t2)
            ).clean()
    ...
    def test_str(self):
        ...
        t = _t()
        sup = Supplier.all_objects.create(tenant=t, name='Forn')
        rpt = RecurringPurchaseOrderTemplate.all_objects.create(
            tenant=t, supplier=sup, branch=_b(t), frequency='monthly'
        )
...
class TestRecurringTemplateItemClean:
    ...
    def test_line_total(self):
        ...
        rti = RecurringTemplateItem(
            tenant=_t(), quantity=Decimal('2'), unit_cost=Decimal('5'), factor=Decimal('3')
        )
...
class TestSalesBatchOperations:
    ...
    def test_close_missing_session_id(self):
        ...
        request = MagicMock()
        request.tenant = t
        request.user = _u(t, 'bc@t.co')
        with pytest.raises(ValueError, match='session_id is required'):
            _route_batch_operation(
                request,
                {
                    'type': 'cash-session:close',
                    'payload': {'closing_amount': 0},
                    'idempotency_key': 'bk',
                },
            )
...
class TestSalesErrorHandler:
    def _vs(self, cls):
        ...
        return cls() if cls == SaleViewSet else CashSessionViewSet()
    ...
    def test_cash_session_required(self):
        ...
        assert (
            self._vs(SaleViewSet)._handle_sales_error(CashSessionRequired('x')).status_code == 409
        )
    ...
    def test_insufficient_returnable(self):
        ...
        assert (
            self._vs(SaleViewSet)
            ._handle_sales_error(InsufficientReturnableQuantity('x'))
            .status_code
            == 409
        )
    ...
    def test_already_cancelled(self):
        ...
        assert (
            self._vs(SaleViewSet)._handle_sales_error(SaleAlreadyCancelled('x')).status_code == 409
        )
    ...
    def test_cash_session_not_found(self):
        ...
        assert (
            self._vs(CashSessionViewSet)._handle_sales_error(ObjectDoesNotExist()).status_code
            == 404
        )
    ...
    def test_cash_session_exists(self):
        ...
        assert (
            self._vs(CashSessionViewSet)._handle_sales_error(OpenCashSessionExists('x')).status_code
            == 409
        )
...
class TestPurchasingServices:
    def test_payload_hash(self):
        ...
        h1 = _payload_hash({'a': 1, 'b': 'x'})
        h2 = _payload_hash({'b': 'x', 'a': 1})
    ...
    def test_advance_schedule(self):
        ...
        t = _t()
        sup = Supplier.all_objects.create(tenant=t, name='SA')
        for freq, delta in [
            ('daily', timedelta(1)),
            ('weekly', timedelta(7)),
            ('biweekly', timedelta(14)),
        ]:
    ...
    def test_advance_no_next_run(self):
        ...
        t = _t()
        sup = Supplier.all_objects.create(tenant=t, name='SN')
        tpl = RecurringPurchaseOrderTemplate.all_objects.create(
            tenant=t,
            supplier=sup,
            branch=_b(t),
    ...
    def test_auto_onboard_new(self):
        ...
        t = _t()
        sup = _run_in_tenant(
            t, lambda: auto_onboard_supplier(tenant=t, cnpj='99988877766655', name='Novo')
        )
    ...
    def test_auto_onboard_existing(self):
        ...
        t = _t()
        existing = Supplier.all_objects.create(tenant=t, name='Ex', cnpj='11122233344455')
        result = _run_in_tenant(
            t, lambda: auto_onboard_supplier(tenant=t, cnpj='11122233344455', name='Outro')
        )
    ...
    def test_summary(self):
        ...
        t = _t()
        result = _run_in_tenant(t, lambda: purchasing_summary(tenant=t))
...
class TestSalesServiceHelpers:
    ...
    def test_close_already_closed(self):
        ...
        user = _u(t, 'ccs@t.co')
        cs = CashSession.all_objects.create(
            tenant=t, branch=_b(t), operator=user, opening_amount=0, status='closed'
        )
        assert (
            close_cash_session(cash_session=cs, closing_amount=0, idempotency_key='cc').pk == cs.pk
        )
    ...
    def test_return_no_items(self):
        ...
        with pytest.raises(ValueError, match='at least one item'):
            create_sale_return(
                tenant=_t(), sale=MagicMock(), items=[], reason='x', idempotency_key='r1'
            )
    ...
    def test_refund_negative(self):
        ...
        with pytest.raises(ValueError, match='positive'):
            create_sale_refund(
                ...
                method='pix',
                amount=Decimal('-5'),
                reason='Teste de valor negativo',
                idempotency_key='rf2',
            )
...
class TestSupportAccessApproveRevoke:
    ...
    def test_approve_not_found(self):
        ...
        vs.format_kwarg = None
        with patch(
            'platform_admin.views.approve_support_access', side_effect=ValueError('not found')
        ):
    ...
    def test_approve_conflict(self):
        ...
        vs.format_kwarg = None
        with patch(
            'platform_admin.views.approve_support_access', side_effect=ValueError('already')
        ):
    ...
    def test_revoke_not_found(self):
        ...
        vs.format_kwarg = None
        with patch(
            'platform_admin.views.revoke_support_access', side_effect=ValueError('not found')
        ):
...
class TestApplyProviderQueryResult:
    ...
    def test_rejected(self):
        ...
        doc = MagicMock()
        doc.attempt_number = 0
        result = MagicMock()
        result.status = 'REJEITADO'
        result.error_reason = 'bad'
...
class TestApplyProductSerializer:
    def test_tracks_inventory_no_stock(self):
        ...
        ser = ApplyProductSerializer(
            data={
                'command_id': 'c',
                'product': {
                    'tracks_inventory': True,
                    'product_kind': 'produto',
                    'name': 'P',
                    'base_unit': str(uuid.uuid4()),
                },
            }
        )
