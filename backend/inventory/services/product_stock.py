    ...
    *,
    ...
    if existing:
        if existing.payload_hash != payload_hash:
            raise ProductStockControlError(
                'Command ID jÃ¡ foi utilizado com payload diferente.',
    ...
    if has_reservations:
        raise ProductStockControlError(
            'DesativaÃ§Ã£o rejeitada: existem reservas ativas.',
    ...
    if has_nonzero_qty:
        raise ProductStockControlError(
            'DesativaÃ§Ã£o rejeitada: existem saldos nÃ£o-zero.',
    ...
    *,
    ...
    for index, stock_entry in enumerate(initial_stocks):
        if not isinstance(stock_entry, dict):
            raise ProductStockControlError(
                f'Estoque inicial invÃ¡lido na posiÃ§Ã£o {index}.',
        ...
        try:
            ...
            raise ProductStockControlError(
                f'LocalizaÃ§Ã£o invÃ¡lida na posiÃ§Ã£o {index}.',
                code='INVALID_INITIAL_STOCK',
                status_code=400,
            ) from None
        try:
            ...
            raise ProductStockControlError(
                f'Quantidade invÃ¡lida na posiÃ§Ã£o {index}.',
                code='INVALID_INITIAL_STOCK',
                status_code=400,
            ) from None
        if not quantity.is_finite() or quantity <= 0:
            raise ProductStockControlError(
                f'Quantidade deve ser positiva na posiÃ§Ã£o {index}.',
                code='INVALID_INITIAL_STOCK',
                status_code=400,
            )
    ...
    if not policies.exists():
        raise ProductStockControlError(
            'Nenhuma polÃ­tica de estoque encontrada para reativar.',
    ...
    for index, stock_entry in enumerate(validated_stocks):
        if not policies.filter(location_id=stock_entry['location_id']).exists():
            raise ProductStockControlError(
                f'LocalizaÃ§Ã£o nÃ£o pertence Ã s polÃ­ticas do produto na posiÃ§Ã£o {index}.',
                code='INVALID_INITIAL_STOCK',
                status_code=400,
            )
    ...
    if validated_stocks:
        ...
        for stock_entry in validated_stocks:
            location_id = cast(UUID, stock_entry['location_id'])
        ...
        if locations_with_history:
            raise ProductStockControlError(
                'Estoque inicial nÃ£o permitido para localizaÃ§Ãµes com histÃ³rico.',
    ...
    if validated_stocks:
        ...
        for stock_entry in validated_stocks:
            location_id = cast(UUID, stock_entry['location_id'])
            quantity = cast(Decimal, stock_entry['quantity'])
            ...
            create_receipt(
                ...
                idempotency_key=idempotency_key,
                actor=actor,
                reason=f'Estoque inicial na reativaÃ§Ã£o de {product.sku}',
            )
