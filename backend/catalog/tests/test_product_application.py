...
from catalog.services.product_extensions import (
    add_product_price_tier,
    upsert_product_fiscal_data,
)
...
def _setup(tenant_name='S22-AS'):
    ...
    product = Product.objects.create(
        tenant=tenant, sku='S22-AS-PROD', name='Test Product', base_unit=unit, category=cat
    )
...
def test_upsert_fiscal_data_creates_when_not_exists():
    ...
    data = _in_tenant(
        tenant,
        lambda: upsert_product_fiscal_data(
            tenant=tenant,
            product=product,
            ncm='12345678',
            origin_code='0',
