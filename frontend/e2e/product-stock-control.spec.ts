...
test('cadastra produto com classificadores, saldo inicial e extensões persistidas', async ({ authenticatedPage }) => {
  ...
  await expect(page.getByTestId('stock-available-value')).toHaveText('25,000000')
  await expect(page.getByTestId('stock-current-value')).toHaveText('25,000000')
  await expect(page.getByTestId('stock-reserved-value')).toHaveText('0,000000')
  ...
  const tierResponsePromise = page.waitForResponse(
    (response) => response.url().includes('/price-tiers/') && response.request().method() === 'POST',
  )
  ...
  await expect(page.getByTestId('price-tier-row')).toBeVisible()
  await page.goto(editUrl)
  await page.getByRole('tab', { name: 'Preços' }).click()
  await expect(page.getByTestId('price-tier-row')).toHaveCount(1)
